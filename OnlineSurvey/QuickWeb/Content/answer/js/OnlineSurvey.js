...
function optionLabelWidth() {
	const $optinLabels = $(".questionnaire_question_item_option_option_label");
	for(let i = 0; i < $optinLabels.length; i++) {
		const $cur = $optinLabels.eq(i);
		const $parent = $cur.parent();
		const width = $parent.css("width").replace("px", "") - 40;
		//$cur.css("width", width + "px");
		$cur.css({
			"cssText":`min-width:${width}px !important`
		});
	}
}
...
$(".firstLevelSelect").change(function() {
	const data = $(this).val();
	const secondLevelSelectData = $(`#secondLevelSelectData_${data}`).val().split(",");
	const $secondSelect = $(this).next();
	$secondSelect.html("");
	var s = "<option value='0'>--请选择--</option>";
	for(let i = 0; i < secondLevelSelectData.length; i++) {
...
$(".paixuti :checkbox").click(function() {
	...
	const $allCheckboxs = $(`#questionItem_${questionid}`).find(":checkbox");
	var maxIndex = 0;
...
function questionShowHide() {
	const $allXialacaidan = $(".questionnaire_question_item_xialacaidan select");
	...
	for(var i = 0; i < $logicSettingDiv.length; i++) {
		const $cur = $logicSettingDiv.eq(i);
		const arrTiaoJian = $cur.attr("logic").split(",");
		if($cur.attr("hassettinglogicdiv") == 0) {//并且关系
			var right = true;
			for(var j = 0; j < arrTiaoJian.length; j++) {
				var optionId = arrTiaoJian[j];

				if($(`input[value=${optionId}]`).is(':checked') == false &&
					allXialacaidanSelectedValue.indexOf(`,${optionId},`) <= 0) {
					right = false;
					break;
				}
			}
		...
		if($cur.attr("hassettinglogicdiv") == 1) {//或者关系
			...
			for(var j = 0; j < arrTiaoJian.length; j++) {
				var optionId = arrTiaoJian[j];
				if($(`input[value=${optionId}]`).is(':checked') ||
					allXialacaidanSelectedValue.indexOf(`,${optionId},`) > 0) {
					right = true;
					break;
				}
...
function changeValidateCode() {
	$("#yzmimg").attr("src",`/Home/GetValidateCode?id=${new Date().getTime()}`);
}
		...
		if($cur.hasClass("juzhenti")) {
			...
			if(juzhenType == 4) {//数字矩阵
				...
				for(var j = 0; j < $inputs.length; j++) {
					...
					if(v != "" && v != null) {
						if((min == 0 && max == 0) ||
							((min != 0 || max != 0) && numberValue >= min && numberValue <= max)) {
							rowSelect++;
						}
...
$(".btnConfirm").click(function() {
	...
	if($firstNotValidateQuestion != null) {
		...
		$.post("/Home/Submit", $("#form1").serialize(), function(data) {
			if(data == "ok") {
				$(".btnConfirm").text("提交成功正在跳转");
				const code = $("input[name=questionnaireCode]").val();
				window.location.href = `/Home/GoingResult/${code}`;
			} else {

...
namespace Quick.Repository
{
    ...
    public partial class BaseRepository<T> : Disposable, IBaseRepository<T> where T : class, new()
    {
        ...
        /// <param name="isAsc">是否升序</param>
        /// <returns>还未执行的SQL语句</returns>
        public virtual IOrderedQueryable<T> LoadOrderedEntities<TS>(Expression<Func<T, bool>> @where, Expression<Func<T, TS>> @orderby, bool isAsc = true, params Expression<Func<T, TS>>[] thenby)
        {
            IOrderedQueryable<T> query;
            if (isAsc)
            {
        ...
        {
            return LoadOrderedEntities(where, orderby, isAsc).ProjectToQueryable<TDto>();
        }
        ...
        {
            return LoadOrderedEntities(where, orderby, isAsc, thenby).ProjectToQueryable<TDto>();
        }
        ...
        {
            return LoadOrderedEntities(where, orderby, isAsc).FromCache(CachePolicy.WithDurationExpiration(TimeSpan.FromSeconds(timespan)));
        }
        ...
        {
            return LoadOrderedEntities(where, orderby, isAsc, thenby).FromCache(CachePolicy.WithDurationExpiration(TimeSpan.FromSeconds(timespan)));
        }
        ...
        {
            return LoadOrderedEntities(where, orderby, isAsc).ProjectToQueryable<TDto>().FromCache(CachePolicy.WithDurationExpiration(TimeSpan.FromSeconds(timespan)));
        }
        ...
        {
            return LoadOrderedEntities(where, orderby, isAsc, thenby).ProjectToQueryable<TDto>().FromCache(CachePolicy.WithDurationExpiration(TimeSpan.FromSeconds(timespan)));
        }
        ...
        {
            return LoadOrderedEntities(where, orderby, isAsc).Cacheable();
        }
        ...
        {
            return LoadOrderedEntities(where, orderby, isAsc, thenby).Cacheable();
        }
        ...
        {
            return LoadOrderedEntities(where, orderby, isAsc).Cacheable().ProjectToQueryable<TDto>();
        }
        ...
        {
            return LoadOrderedEntities(where, orderby, isAsc, thenby).Cacheable().ProjectToQueryable<TDto>();
        }
        ...
        {
            return await LoadOrderedEntities(where, orderby, isAsc).FromCacheAsync(CachePolicy.WithDurationExpiration(TimeSpan.FromSeconds(timespan))).ConfigureAwait(true);
        }
        ...
        {
            return await LoadOrderedEntities(where, orderby, isAsc, thenby).FromCacheAsync(CachePolicy.WithDurationExpiration(TimeSpan.FromSeconds(timespan))).ConfigureAwait(true);
        }
        ...
        {
            return await LoadOrderedEntities(where, orderby, isAsc).ProjectToQueryable<TDto>().FromCacheAsync(CachePolicy.WithDurationExpiration(TimeSpan.FromSeconds(timespan))).ConfigureAwait(true);
        }
        ...
        {
            return await LoadOrderedEntities(where, orderby, isAsc, thenby).ProjectToQueryable<TDto>().FromCacheAsync(CachePolicy.WithDurationExpiration(TimeSpan.FromSeconds(timespan))).ConfigureAwait(true);
        }
        ...
        {
            return DataContext.Set<T>().Where(@where).Update(@do);
        }
        ...
        {
            return await DataContext.Set<T>().Where(@where).UpdateAsync(@do);
        }
